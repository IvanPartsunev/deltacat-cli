...
def create_namespace_cmd(name: Annotated[str, typer.Argument(help='Namespace name to create')]) -> None:
    ...
    try:
        ...
        console.print(f'{get_emoji("loading")} Creating namespace "[cyan]{name}[/cyan]" in catalog "[yellow]{catalog_name}[/yellow]"...')

        catalog_context.get_catalog()
        create_namespace(namespace=name, catalog=catalog_name)

        console.print(f'{get_emoji("success")} Namespace "[bold cyan]{name}[/bold cyan]" created successfully', style='green')
