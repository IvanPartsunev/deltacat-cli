...
def create_namespace_cmd(name: Annotated[str, typer.Option(help='Namespace name to create')]) -> None:
    """Create a new namespace in the current catalog."""
    try:
        catalog_name, _ = catalog_context.get_catalog_info(silent=True)
        console.print(
            f'{get_emoji("loading")} Creating namespace "[cyan]{name}[/cyan]" in catalog "[yellow]{catalog_name}[/yellow]"...'
        )
        ...
        console.print(
            f'{get_emoji("success")} Namespace "[bold cyan]{name}[/bold cyan]" created successfully', style='green'
        )
