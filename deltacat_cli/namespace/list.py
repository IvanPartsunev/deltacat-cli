...
def list_namespace_cmd() -> None:
    ...
    try:
        ...
        console.print(f'🔄 Listing Namespaces in Catalog "[cyan]{catalog_name}[/cyan]"...')

        all_namespaces = list_namespaces(catalog=catalog_name).all_items()

        if not all_namespaces:
            console.print('📭 No namespaces found in this catalog', style='yellow')
            return

        for namespace in all_namespaces:
          print_json(source_type='namespace', data=namespace)

        console.print(f'✅ Found {len(all_namespaces)} namespace(s)', style='green')
        console.print()
